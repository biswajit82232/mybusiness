...
export function PaginatedVendorList({ filtered, onOpenVendor, emptyState, alphaHeaders }) {
      ...
      {visible.map((c, i) => {
        ...
        return (
          <Fragment key={c.key}>
            ...
            <button type="button" className="customer-row" onClick={() => onOpenVendor(c.name)}>
              <div className={`avatar ${avatarColor(c.name)}`}>{avatarInitials(c.name)}</div>
              <div className="customer-info">
                <div className="customer-name">{c.name}</div>
                <div className="customer-meta">
                  <div className="customer-kpi">
                    <span className="customer-kpi-lbl">Total purchases</span>
                    <span className="customer-kpi-val">{money(c.totalPurchases)}</span>
                  </div>
                  <div className="customer-kpi">
                    <span className="customer-kpi-lbl">Payables</span>
                    <span className={`customer-kpi-val ${c.totalOutstanding > 0 ? "due" : "paid"}`}>{money(c.totalOutstanding)}</span>
                  </div>
                  <div className="customer-kpi">
                    <span className="customer-kpi-lbl">Bills</span>
                    <span className="customer-kpi-val">{c.purchaseCount}</span>
                  </div>
                </div>
              </div>
              <IcChevR />
